...
import { uploadImage, EditUser, deleteImage } from '../api/internal'
...
const Navbar = ({ setAlert }) => {
  ...
  const handleDeleteImage = async () => {
    ...
    const response = await deleteImage(user)
    if (response.status == 200) {
            ...
            <div className="bg-white absolute rounded-3xl p-4 pb-10 w-[80vw] mb-10 sm:w-3/4 h-fit">
              ...
              <div>
                <div className='flex h-fit   flex-col   px-4 ' >
                  ...
                  <div className="relative w-32 h-32 rounded-full overflow mx-auto mb-4">
                    <input ref={fileInputRef} type="file" id="fileInput" accept="image/*" onChange={handleFileChange} className="absolute inset-0 w-full h-full  opacity-0 cursor-pointer" />
                    ...
                    <h1 onClick={editBtnHandler} className='text-center absolute left-[8px] bottom-[1px] bg-[#45454590] py-2 flex items-center justify-center' style={{
                      clipPath: "polygon(0 14px, 100% 14px, 114% 100%, 0 124%, 15px 126%)",
                      borderRadius: "0 0 50% 50% / 0 0 100% 100%",
                      width: "112px",
                      height: '45px',
                      cursor: "pointer",
                    }}>
                      <span className='mt-2 text-lg text-white font-semibold'>
                        edit
                      </span>
                    </h1>
                    <div onClick={() => handleDeleteImage()} className='p-3 bg-[#ffcdcd] hover:bg-[#fcd5d5]  flex justify-center items-center w-10 absolute top-0 right-0 rounded-full'>
