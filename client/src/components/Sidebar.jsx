...
const Sidebar = ({ side, setSide, display, setDisplay, setAlert }) => {
  ...
  return (
    <>
      {display && <aside className={`${side ? "" : "pr-12"} flex flex-col gap-1 p-3 pt-16 transition-all z-10 3xl:text-2xl 3xl:pr-3 bg-[#3093b4] text-xl sm:h-[100vh] text-[#fff] fixed h-[100vh]`}>
        {(role == 'admin' || role == 'HR') && 
        <NavLink title="dashboard" to={"/"} className="flex items-center gap-3 p-2 ">
          <svg width={`${side ? "35" : "25"}`} height={`${side ? "35" : "25"}`} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <span className={`${side ? "hidden absolute" : "block"} `}>
            Dashboard
          </span>
        </NavLink>}
        {role == 'admin' && <NavLink title="Users" to={"/users"} className="flex items-center gap-3 p-2">
          <svg width={`${side ? "35" : "25"}`} height={`${side ? "35" : "25"}`} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <NavLink title="Data Entry" onClick={() => setClickedDataEntry(!clickedDataEntry)} className="flex items-center gap-3 p-2 py-1 ">
          <svg width={`${side ? "35" : "25"}`} height={`${side ? "35" : "25"}`} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        {(role == 'admin' || role == 'HR') && <NavLink title="Records" to={"/records"} className="flex items-center gap-3 p-2">
        ...
        {role == 'admin' &&
          <div className='justify-end'>
            <NavLink title="Logout" to={"/settings"} className="flex items-center gap-3 p-2">
        ...
        <div className="h-screen flex items-end">
          ...
          <NavLink title="Logout" to={"/logout"} className="flex items-center  gap-3 p-2">
          <svg width={`${side ? "35" : "25"}`} height={`${side ? "35" : "25"}`} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
